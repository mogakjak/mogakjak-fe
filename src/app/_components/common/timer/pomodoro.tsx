...
>(function PomodoroDial({ minutes = 60, className, onComplete }, ref) {
  ...
  return (
    <section
      className={clsx(
        "w-full h-full rounded-2xl flex items-center justify-start gap-7 pl-3 bg-neutral-50",
        className
      )}
      data-state={isUnset ? "inactive" : running ? "running" : "paused"}
    >
      <div className="w-[128px] h-[128px] relative grid place-items-center">
        <div className="w-[128px] h-[128px]  rounded-full border-3 border-gray-200" />
        <svg
          width={dial}
          height={dial}
